...
export default function Lookbook() {
  ...
  const filteredOutfits = useMemo(() => {
    ...
    return outfitsData.outfits.filter(outfit => {
      ...
      const matchActivity = selectedActivities.length === 0 ||
        selectedActivities.some(a => outfitActivities.includes(a));

      const matchItem = !selectedItem ||
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Cari kegiatan, item, atau gaya..."
              value={search}
          ...
          <button
            ...
            className={showFilters ? 'btn-accent' : 'btn-ghost'}
          >
          ...
          <div className="animate-slide-down space-y-5 p-5 rounded-2xl" style={{ background: 'var(--bg-card)', border: '1px solid var(--border-light)' }}>
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {allStyles.map(style => (
                  <button key={style} onClick={() => toggleStyle(style)} className={`chip ${selectedStyles.includes(style) ? 'active' : ''}`}>
                    {style}
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {allActivities.map(activity => (
                  <button key={activity} onClick={() => toggleActivity(activity)} className={`chip ${selectedActivities.includes(activity) ? 'active' : ''}`}>
                    {activity}
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="relative inline-block">
                <select
                  value={selectedItem}
                  onChange={(e) => { setSelectedItem(e.target.value); setRandomOutfit(null); }}
                ...
                >
                  ...
                  <optgroup label="Atasan">
                    {allTops.map(t => <option key={t} value={t}>{t}</option>)}
                  </optgroup>
                  <optgroup label="Bawahan">
                    {allBottoms.map(b => <option key={b} value={b}>{b}</option>)}
                  </optgroup>
                </select>
      ...
      <div className="flex items-center justify-between">
        ...
        {randomOutfit && (
          <button onClick={() => setRandomOutfit(null)} className="btn-ghost text-xs">
            Tampilkan Semua
          </button>
        )}
      </div>
...
function OutfitCard({ outfit, isHighlighted }) {
  ...
  const images = items.map(i => ({ ...i, src: imageMap[i.name] })).filter(i => i.src);
  ...
  return (
    ...
    >
      {/* Flat-lay collage — uniform 2x2 grid, all same aspect ratio */}
      <div className="grid grid-cols-2 gap-1 p-2" style={{ background: 'linear-gradient(145deg, #f5f3ef 0%, #eae7e1 100%)' }}>
        ...
        {/* Fill remaining slots with watch if less than 4 images */}
        {images.length < 4 && outfit.watch !== '-' && imageMap[outfit.watch] && (
          <div className="relative overflow-hidden rounded-lg group/item" style={{ aspectRatio: '3/4' }}>
            <img
              src={imageMap[outfit.watch]}
              alt={outfit.watch}
              className="w-full h-full object-cover transition-transform duration-500 group-hover/item:scale-105"
              loading="lazy"
            />
        ...
        <div className="space-y-1.5">
          {items.map((item, idx) => (
            <div key={idx} className="flex items-baseline gap-2">
              <span className="text-[10px] uppercase tracking-[0.1em] font-semibold w-12 flex-shrink-0" style={{ color: 'var(--text-muted)' }}>
                {item.label}
              </span>
